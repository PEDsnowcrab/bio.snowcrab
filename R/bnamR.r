...
#' This file needs to be requested annually to include past year. Remove BNAM .mat file and replace with new
...
bnamR = function(redo=F , x, bnam_location = file.path(project.datadirectory('bio.snowcrab'),'data','bnam','raw.matlab'),
                 outfile= file.path(project.datadirectory('bio.snowcrab'),'data','bnam','bnams.reshape.rds')) {
                
  if(redo){
    require(R.matlab)
    x=list.files(bnam_location)[1] #should only be one, don't need to change file name annually
    a = readMat(paste(bnam_location, x, sep="/"))
    #a=merge(a, b)
